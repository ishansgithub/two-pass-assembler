...
int main(int argc, char **argv)
{
    ...
    /* 
        ...
        sample fix for the label errors I was getting 
    ...
    */
    ...
    for (auto& label : labelInfo) {
        if (label.second.first == -1) {
            ...
            // If label is declared but never used, log a warning
            warningList.push_back({label.second.second, "Unused Label"});
        }
    }

    // Sorting the errors and warnings by line number (simple sort function calling)
